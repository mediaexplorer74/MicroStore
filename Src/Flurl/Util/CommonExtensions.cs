...
namespace Flurl.Util
{
	...
	public static class CommonExtensions
	{
        ...
        private static IEnumerable<(string Name, object Value)> ObjectToKV(object obj)
        {
			...
			//return Enumerable.Empty<(prop.Name, val) >();
		}
        ...
        private static IEnumerable<(string Key, object Value)> CollectionToKV(IEnumerable col) 
		{
			...
			{
				object prop = null; //obj.GetType().GetProperty(name);
				object field = null; //obj.GetType().GetField(name);

				if (prop != null) 
				{
					value = null;//prop.GetValue(obj, null);
					return true;
				}
				if (field != null) 
				{
					value = null;//field.GetValue(obj);
					return true;
				}
				value = null;
				return false;
