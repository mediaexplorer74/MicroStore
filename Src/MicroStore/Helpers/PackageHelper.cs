...
namespace MicroStore.Helpers
{
    public static class PackageHelper
    {
        ...
        public static async Task<bool> InstallPackage(
            PackageInstance package, ProductDetails product, bool? useAppInstaller = null)
        {
            ToastNotification finalNotif = GenerateInstallSuccessToast(package, product);
            bool isSuccess = true;
            try
            {
                (await DownloadPackage(package, product, false)).Deconstruct(
                    out var installer, out var progressToast);
                ...
                if (Settings.Default.UseAppInstaller || (useAppInstaller.HasValue && useAppInstaller.Value))
                {
                    ...
                    switch (await Launcher.QueryUriSupportAsync(launchUri, LaunchQuerySupportType.Uri))
                    {
                        case LaunchQuerySupportStatus.Available:
                            ...
                            if (!isSuccess)
                                finalNotif = GenerateInstallFailureToast(package, product, 
                                    new Exception("Failed to launch App Installer."));
                            break;

                        case LaunchQuerySupportStatus.AppNotInstalled:
                            finalNotif = GenerateInstallFailureToast(package, product, 
                                new Exception("App Installer is not available on this device."));
                            isSuccess = false;
                            break;

                        case LaunchQuerySupportStatus.AppUnavailable:
                            finalNotif = GenerateInstallFailureToast(package, product, 
                                new Exception("App Installer is not available right now, try again later."));
                            isSuccess = false;
                            break;
                        ...
                        default:
                            finalNotif = GenerateInstallFailureToast(package, product, 
                                new Exception("An unknown error occured."));
                            isSuccess = false;
                            break;
                    }
                }
            ...
            {
                ToastNotificationManager.GetDefault().CreateToastNotifier().Show(
                    GenerateInstallFailureToast(package, product, ex));
                return false;
            }
        }

        public static async Task<bool> InstallPackage(ProductDetails product, bool? useAppInstaller = null,
            Action<ProductDetails> gettingPackagesCallback = null, 
            Action<ProductDetails> noPackagesCallback = null,
            Action<ProductDetails> packagesLoadedCallback = null, 
            Action<ProductDetails, PackageInstance> packageInstalledCallback = null)
        {
            var culture = CultureInfo.CurrentUICulture;

            gettingPackagesCallback?.Invoke(product);

            var dcathandler = new StoreLib.Services.DisplayCatalogHandler(
                DCatEndpoint.Production, new StoreLib.Services.Locale(culture, true));
        ...
        public static async Task<Tuple<StorageFile, ToastNotification>> DownloadPackage(
            PackageInstance package, ProductDetails product,
            bool hideProgressToastWhenDone = true, 
            string filepath = null)
        {
            ...
            download.RangesDownloaded += (op, args) =>
            {
                ToastNotificationManager.GetDefault().CreateToastNotifier().Update(
                    new NotificationData(new Dictionary<string, string>()
                    {
                     { 
                      "progressValue", 
                         ((double)op.Progress.BytesReceived / op.Progress.TotalBytesToReceive).ToString() },
                            { "progressStatus", 
                         "Downloading..." 
                     }
                    }),
            ...
            */
                ...
                switch (magicNumber)
                {
                    ...
                    case 0x504B0304:
                        using (var archive = ZipFile.OpenRead(filepath))
                        {
                            ...
                            if (defaults.Any(defaults => defaults.Attribute("ContentType").Value 
            ...
            {
                Debug.WriteLine("[i] Deleting " + contentTypeFilepath  + "...");
                try
                {
                ...
                {
                    Debug.WriteLine("[!] " + contentTypeFilepath + "deleting error (maybe, installing process hold this file?)");
                }
            }
            ...
            if (extension != string.Empty)
            {
                try
                {
                    await destinationFile.RenameAsync(destinationFile.Name + extension, 
                        NameCollisionOption.ReplaceExisting);
                }
                catch (Exception ex) 
                {
                    Debug.WriteLine("[ex] " + ex.Message + "[" + destinationFile.Name + "]");
                }
            }
        ...
        public static async Task<Tuple<StorageFile, ToastNotification>> DownloadPackage(
            ...
            Action<ProductDetails> noPackagesCallback = null,
            Action<ProductDetails> packagesLoadedCallback = null, 
            Action<ProductDetails, PackageInstance, StorageFile, ToastNotification> packageDownloadedCallback 
            = null)
        {
            ...
            StoreLib.Services.DisplayCatalogHandler dcathandler = new StoreLib.Services.DisplayCatalogHandler(
                DCatEndpoint.Production, new StoreLib.Services.Locale(culture, true));

            await dcathandler.QueryDCATAsync(product.ProductId);
            IList<PackageInstance> packs = await dcathandler.GetMainPackagesForProductAsync();
            string packageFamilyName = dcathandler.ProductListing.Product.Properties.PackageFamilyName;
            ...
            if (packs != null)
            {
                PackageInstance package = GetLatestDesktopPackage(packs.ToList(), packageFamilyName, product);
                if (package == null)
                {
                    noPackagesCallback?.Invoke(product);
                    return null;
        ...
        public static async Task<bool> IsAppInstalledAsync(string packageName)
        {
            try
            {
                ...
                LaunchQuerySupportStatus result = await Launcher.QueryUriSupportAsync(
                    dummyUri, LaunchQuerySupportType.Uri, packageName);
                switch (result)
                {
                    case LaunchQuerySupportStatus.Available:
            ...
            var content = new ToastContent()
            {
                Visual = new ToastVisual()
                {
                    BindingGeneric = new ToastBindingGeneric()
                    {
                        ...
                        {
                            Source = product.Images.FindLast(i =>
                                     i.ImageType == MicrosoftStore.Enums.ImageType.Logo).Url
                        }
                    }
                },
        ...
        public static ToastNotification GenerateDownloadSuccessToast(
            PackageInstance package, 
            ProductDetails product, StorageFile file)
        {
            var content = new ToastContentBuilder().SetToastScenario(ToastScenario.Reminder)
                ...
                //.AddAppLogoOverride(product.Images.FindLast(i
                //        => i.ImageType == MicrosoftStore.Enums.ImageType.Logo).Uri, addImageQuery: false)
                .Content;
            return new ToastNotification(content.GetXml());
        }

        public static ToastNotification GenerateDownloadFailureToast(PackageInstance package, 
            ProductDetails product)
        {
            var content = new ToastContentBuilder().SetToastScenario(ToastScenario.Reminder)
                .AddToastActivationInfo($"action=viewEvent&eventId={package.PackageMoniker}", 
        ...
        public static ToastNotification GenerateInstallSuccessToast(PackageInstance package, 
            ProductDetails product)
        {
            var content = new ToastContentBuilder().SetToastScenario(ToastScenario.Reminder)
                .AddToastActivationInfo($"action=viewEvent&eventId={package.PackageMoniker}", 
        ...
        public static ToastNotification GenerateInstallFailureToast(PackageInstance package,
            ProductDetails product, Exception ex)
        {
            var content = new ToastContentBuilder().SetToastScenario(ToastScenario.Reminder)
                .AddToastActivationInfo($"action=viewEvent&eventId={package.PackageMoniker}",
                ToastActivationType.Foreground)
